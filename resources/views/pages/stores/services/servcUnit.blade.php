<div class="ctr-listDetClthsServc mt-2 max-h-64 pr-2 overflow-y-scroll">
    <div class="cListDetClthsServc sm:space-y-1 space-y-3 text-xs sm:text-sm">
        @php
            $countActiveServc = 0;
            $listUnit = isset($serviceUnits) ? count($serviceUnits) : 0;
        @endphp
        @if(isset($serviceUnits) && (count($serviceUnits) > 0))
            @foreach ($serviceUnits as $units)
                @php
                    $units->active ? $countActiveServc++ : $countActiveServc;
                @endphp
                <div class="itm-detServc flex flex-col sm:flex-row gap-2 p-2 rounded-xl border-2 border-gray-400 has-[:checked]:border-pink-300 has-[:checked]:bg-pink-200 group overflow-hidden" data-idServcClothes="{{ $units->idUnit }}">
                    <div class="detServc w-full flex items-center gap-2">
                        <div class="actvThsServc">
                            <input type="checkbox" class="checkActvThsClthsServc rounded-[100%] p-2" {{ $units->active ? 'checked' : '' }}>
                        </div>
                                ...
                                <div class="inpFldDtValThsServc w-full flex items-center gap-2">
                                    ...
                                    <div class="inpFld block w-full">
                                        <input 
                                            ...
                                            spellcheck="false"
                                            value="{{ number_format($units->price) }}">
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <label for="actvAllThsClthsServc" class="block w-fit mt-2">
            <div class="ctr-thsServcActv">
                <div class="cThsServcActv block w-16 h-6 rounded-full cursor-pointer border border-black relative bg-white has-[:checked]:bg-[#ff92b6] has-[:checked]:border-pink-700 *:transition-all">
                    <input type="checkbox" name="actvAllThsClthsServc" id="actvAllThsClthsServc" class="peer sr-only" {{ $listUnit > 0 && $countActiveServc === $listUnit ? 'checked' : '' }}>
